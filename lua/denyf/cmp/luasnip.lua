...
local cfg = pRequire("denyf").config.cmp

local pathUtils = require("denyf.utils.path")
...
pRequire("luasnip.loaders.from_lua").load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "denyf", "cmp", "snippets", "lua"),
})
pRequire("luasnip.loaders.from_vscode").lazy_load({
  paths = pathUtils.join(pathUtils.getConfig(), "lua", "denyf", "cmp", "snippets", "vscode"),
})
